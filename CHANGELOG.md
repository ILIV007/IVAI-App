...
- Loopback, private-LAN and HTTP local model endpoints remain blocked pending a dedicated trust mode, narrow Android cleartext allowlist, explicit warning UX and device evidence.
...
- Complete the separately reviewed local endpoint trust mode for user-operated Ollama, LM Studio, vLLM and compatible local/LAN servers before claiming local-model connectivity.
