...
- Bounded Local Agent Alpha with calculation, current-time, explicit one-time project-write approval, limits, cancellation, run trace, target validation, and process-death approval recovery.
...
- No always-allow Agent write behavior and no replay of pending writes after process death.
...
- Complete the remaining safe read-only Agent tool scope or explicitly keep it excluded.
