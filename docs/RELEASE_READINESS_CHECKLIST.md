...
| Independent unit execution | Forced-rerun log and report totals | Zero failures, errors, and skipped tests. Current baseline is 125 tests and must not regress without a documented review. | Pending |
...
| Target failure recovery | UI explains recovery and never auto-changes provider, target, permission, or local file. | P0 | Pending |
