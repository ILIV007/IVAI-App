...
>
> **Decision:** IVAI will prepare for Skills and Model Context Protocol (MCP) as **separate, user-controlled capability planes**. Their implementation begins only after the current Phase 7.5 evidence gates and Alpha release gates are closed, and each subphase remains independently reviewable. The [Phase 8.0 Capability Consent and Traceability Matrix](PHASE8_0_CAPABILITY_CONSENT_MATRIX.md) is the canonical cross-reference for future state, consent, redaction, and evidence terminology; the [Phase 8.0 Skills and MCP Threat Model](PHASE8_0_SKILLS_MCP_THREAT_MODEL.md) maps future assets, trust boundaries, abuse cases, mitigations, and required evidence; the [Phase 8.0 Prototype Review Specification](PHASE8_0_PROTOTYPE_REVIEW_SPECIFICATION.md) turns those decisions into inert review scenarios and acceptance criteria.
