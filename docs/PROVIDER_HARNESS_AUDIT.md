...
| No default provider or Gemini-first product identity | Sidebar is provider-neutral, but the add-provider dialog initially selects Gemini. | **UI deviation** | Start from an unselected connection mode or preset choice; never create or privilege a provider automatically. |
