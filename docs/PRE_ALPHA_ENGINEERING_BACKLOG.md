# Pre-Alpha Engineering Backlog and ER-13 Handoff

> **Status:** Prepared from clean `main` commit `e3d3bad` after the post-ER-01 global review, then updated through ER-13. ER-01 through ER-05, ER-10, ER-14, ER-15, ER-22, ER-24, and ER-25 are merged; ER-13 passed focused/release/debug/full deterministic validation locally, with protected merge pending. This document does not close Phase 7.5, authorize an Alpha release, or add any runtime capability.

## Current Decision

The protected `main` baseline at `5779853` remains synchronized and green. The ER-13 implementation branch has 142 passing tests and zero lint issues after clean debug/release validation. After its protected merge, subsequent P3 work proceeds only from an evidence-confirmed candidate. ER-13 remains intentionally separate from Provider public API semantics, live network validation, endpoint/network policy, signing, publication, device validation, MCP, and Alpha approval.

| Decision | Status |
|---|---|
| Next implementation increment after ER-13 merge | Evidence-confirmed P3 candidate only |
| Approved `main` baseline before ER-13 | `5779853` (PR #64) |
| ER-13 branch deterministic quality baseline | 142 tests; 0 failures, errors, skipped tests, and lint issues; successful R8 release APK/mapping |
...
| 10 | ER-13 — local credential availability naming | Call-site audit and no-transport regression are locally green; active gates now use `checkStoredCredentialAvailability`; protected merge pending. | Naming must not imply a network validation or obscure the BYOK/local boundary. |
